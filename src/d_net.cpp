...
struct TicSpecial //adicionar "static" aqui se der ruim!
{
...
void NetUpdate (void)
{
	...
	for (i = 0; i < doomcom.numnodes; i++)
	{
		...
		if (numtics > 0)
		{
					...
					if (l == 0)
					{
						...
						*/
						...
						if (specials.used[start])
						{
							...
							size_t specialSize = specials.used[start];
			...
			int packetSize = int(cmddata - netbuffer);

			if (packetSize > 14000)
			{
				Printf(
					TEXTCOLOR_RED
					"VOIP: PACKET TOO LARGE: %d / %d bytes\n",
					packetSize,
					14000
				);
			}

			HSendPacket(i, packetSize);
		}
		else
		{
			HSendPacket (i, k);
